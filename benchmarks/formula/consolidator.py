"""Formula consolidation: the consolidator call of the paper's runs (default) or a plain append.
...
SectionedPlaybook.parse_ops_response / apply_add_ops. An unusable reply, a text the operations cannot be
applied to or a failed call leaves the playbook unchanged; the runner then records store_decision "consolidator_error".
"""
import json
import time
...
TOKEN_BUDGET = 80000
CONSOLIDATOR_MAX_TOKENS = 8192
CONSOLIDATOR_PROMPT = read_prompt("consolidator/formula.txt")
...
class LLMConsolidator:
    """`--consolidator llm` (default). `history` keeps one record per call (usage, outcome, new ids) until
    the runner drains it."""

    def __init__(self, llm, n_tasks: int, adaptive: bool, token_budget: int = TOKEN_BUDGET,
                 max_tokens: int = CONSOLIDATOR_MAX_TOKENS):
    ...
    def build_prompt(self, playbook: SectionedPlaybook, episode: EpisodeState, task: dict) -> str:
        return CONSOLIDATOR_PROMPT.format(token_budget=self.token_budget, current_step=task["task_index"] + 1,
    ...
    def consolidate(self, playbook: SectionedPlaybook, episode: EpisodeState, task: dict, traj: Trajectory) -> str:
        ...
        try:
            ...
            ops = playbook.parse_ops_response(text)
            if ops is None:
