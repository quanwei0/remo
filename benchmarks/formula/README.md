...
| `Consolidate` | `consolidator.CuratorConsolidator` (default): `prompts/consolidator/formula.txt` with token budget 80000, step / total, playbook stats, the lesson built from the accepting round, the playbook and the question; ADD operations applied through `SectionedPlaybook`. `--consolidator append`: the key insight becomes one bullet in OTHERS, no call |
| Alg. 1 / 2 | `remo.ReMoAgent`, sequential over the 200 questions; `run_formula.FormulaAgent` adds per-round bookkeeping, `curator_error`, and the read-only phase of `--freeze-after` |
...
- Parameters of the runs: temperature 0, one user message per call, `max_tokens` 8192 for solver, critic and
  curator, K = 3, freeze window 20 / rate 0.1 / probe 20, redundant lessons reinforced (`helpful+1` on every cited id).
...
- Lesson sent to the curator: `[validated: <gate>] The final answer passed independent review[ after N rounds of
...
- A failed solver or critic call ends the episode (not admitted; the round is counted); a failed curator call or an
  unusable curator reply leaves the playbook unchanged and records `store_decision: curator_error`.
...
consolidator.py   lesson text, CuratorConsolidator, AppendConsolidator
...
`--consolidator {curator,append}`, `--freeze-w 20 --freeze-rho 0.1 --probe-p 20`, `--token-budget 80000`,
...
the playbook is injected but nothing changes: no curator call, no bullet added or reinforced, and the saturation
