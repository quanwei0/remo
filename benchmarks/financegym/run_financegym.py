...
/ gate / memory_decision, playbook add/reinforce, persistence — happens under ONE asyncio.Lock.
...
Episodes whose final report has fewer than --min-docs fetched documents are NOT saved, so a later
run redoes them (when the embedding service dies mid-run every task returns 0 docs).
...
from benchmarks.financegym.common import (BENCH_FILE, DEFAULT_EMBED_URL, DEFAULT_MODEL, DEFAULT_PIT_URL,   # noqa: E402
                                          EMBED_MODEL, INJECT_CAP_CHARS, MIN_DOCS, MODES, PLAYBOOK_PREFIX,
                                          CRITIC_MEMORY_CAP_CHARS, FinanceGymCritic, LLMConsolidator,
                                          is_critic_failure, load_tasks, make_config, read_episodes, summarize_run)
...
class RunState:
    ...
    def __init__(self, cfg: RemoConfig, run_dir: str, consolidator=None, freeze_after: int | None = None,
                 baseline: bool = False):
        self.cfg, self.run_dir, self.freeze_after, self.baseline = cfg, run_dir, freeze_after, baseline
        ...
        self.consolidator = consolidator or AppendConsolidator()
    ...
    async with sem:
        ...
        for r in range(1, cfg.K + 1):
            ...
            rounds_full.append({"round": r, "report": traj.answer, **{k: v for k, v in traj.meta.items()
                                ...
                                "critic_raw": refl.raw})
            async with rs.lock:
                action = rs.policy.after_round(st, traj.completed, refl)
        ...
        # final report: the last round with a non-empty report (an accepted round is always last)
        final = next((t for t in reversed(trajs) if t.answer.strip()), trajs[-1])
        ...
        async with rs.lock:
            ...
            # accepted only because the critic CALL failed (fallback verdict): nothing was verified, so no lesson —
            # not even an earlier round's (EpisodeState.lesson falls back to it) — may enter the memory
            critic_failed = critic is not None and st.admitted and is_critic_failure(st.last)
            if not cfg.use_memory:
                ...
                decision = rs.policy.memory_decision(st, task_index, cited)
            entry_id, pending_lesson = "", None
            if decision == "stored":
                if hasattr(rs.consolidator, "condense"):     # LLM consolidator: the model call runs outside the lock
                    pending_lesson = st.lesson()
                else:
                    entry_id = rs.consolidator.consolidate(rs.playbook, st.lesson(), task, final)
            elif decision == "reinforced":
                if not rs.playbook.reinforce(cited):
                    decision = "discarded"
                entry_id = cited
        if pending_lesson is not None:
            entry_text = await rs.consolidator.condense(pending_lesson, task, final)
            async with rs.lock:
                entry_id = rs.playbook.add(entry_text)
        async with rs.lock:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--base-url", default=None,
                   help="OpenAI-compatible base url for the harness backbone, the page reader (unless "
                        "FH_VLLM_READER_BASE_URL is set), the critic and the consolidator "
    ...
    p.add_argument("--consolidator", default="append", choices=["append", "llm"],
                   help="append = the lesson itself becomes the entry; llm = one model call rewrites it as one general line")
    # FinanceGym-specific
...
def main(argv=None) -> int:
    ...
    critic = None if baseline else FinanceGymCritic(client, args.model, adaptive=cfg.adaptive,
    ...
    rs = RunState(cfg, args.out, consolidator=consolidator, freeze_after=args.freeze_after, baseline=baseline)
    ...
    if args.consolidator == "llm":
        summary["consolidator"] = {"calls": consolidator.calls, "failures": consolidator.failures}
