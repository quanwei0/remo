...
# long ReAct steps. TOOLS=1 adds the tool-call parser FinanceHarness needs (qwen3_xml: Qwen3.5 emits <tool_call><function=...><parameter=...> XML, not the hermes JSON form); Formula and AppWorld run without it.
PORT=${PORT:-8127}; TP=${TP:-4}; UTIL=${UTIL:-0.9}; TOOLS=${TOOLS:-0}; MODEL=${MODEL:-Qwen/Qwen3.5-27B}
TOOL_FLAGS=""; [ "$TOOLS" = "1" ] && TOOL_FLAGS="--enable-auto-tool-choice --tool-call-parser qwen3_xml"
